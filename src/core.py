...
def extract_tsfresh_features(df: pd.DataFrame, column_id: str='id', column_sort: str='time', column_value: str='value', default_fc_parameters: dict | None=None) -> pd.DataFrame:
    """Extract features using TSFresh."""
    if default_fc_parameters is None:
        default_fc_parameters = ComprehensiveFCParameters()
    extracted_features = extract_features(df, column_id=column_id, column_sort=column_sort, column_value=column_value, default_fc_parameters=default_fc_parameters, impute_function=impute)
    return extracted_features

def select_relevant_features(X: pd.DataFrame, y: pd.Series, fdr_level: float=0.05) -> pd.DataFrame:
...
def plot_time_series(df: pd.DataFrame, output_path: Path, n_series: int=3, plot: bool=False):
    ...
    for i, series_id in enumerate(df['id'].unique()[:n_series]):
        ...
        axes[i].plot(series_data['time'], series_data['value'], color='#4A90A4', linewidth=1.2)
        axes[i].set_xlabel('Time')
        axes[i].set_ylabel('Value')
    plt.suptitle('Sample Time Series for Feature Extraction', fontsize=12, y=0.98, color='0.2')
